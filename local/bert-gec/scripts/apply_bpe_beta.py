...
class BPE(object):
    ...
    def process_lines(self, context, outfile, dropout=0, num_workers=1):
        # 直接把整个context(str)写到output
        ...
        if num_workers == 1:
            new_context = _process_lines(self, context, outfile, dropout, 0, 0)
        elif num_workers > 1:
...
def _process_lines(bpe, context: str, outfile, dropout, begin, end):
    ...
    process_context_lis = [bpe.process_line(s, dropout) for s in context_lis]
    new_context = "\n".join(process_context_lis)
    # fo.write(bpe.process_line(context, dropout))
    if isinstance(outfile, str):
        fo.close()
...
def bpe_process(input_str: str):
    ...
    bpe = BPE(args.codes, args.merges, args.separator, vocabulary,
              args.glossaries)  # this object use to process input string in the process_lines method
    ...
    # else:
    new_context = bpe.process_lines(args.input, args.output, args.dropout, args.num_workers)

    return new_context


if __name__ == '__main__':
    # TODO: need to chage the path of bpe8000
    ...
    new_context = bpe_process(context)
    print(context)
    print("==============================")
    print(new_context)
    wait=True
